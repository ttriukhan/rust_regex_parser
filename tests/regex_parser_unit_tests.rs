...
mod regex_parser_unit_tests {
    ...
    fn test_rsymb_on_correct() -> anyhow::Result<()> {
        ...
        for input in correct_inputs {
            let parsed_data = RegexGrammar::parse(Rule::rsymb, input)?.next().ok_or_else( || anyhow! ("no needed pair"))?;
    ...
    fn test_rsymb_on_incorrect() -> anyhow::Result<()> {
        ...
        for input in incorrect_inputs {
            ...
            assert!(parsed_result.is_err(), "Expected error for input: {}", input);
        }
        Ok(())
    }
    ...
    fn test_prime_on_correct() -> anyhow::Result<()> {
        let correct_inputs = ["a","b", ".", ",", "т", "а", "н", "я", "(a)", "(a|b)", "(a*)", "(a|(b*))"];
        for input in correct_inputs {
            let parsed_data = RegexGrammar::parse(Rule::prime, input)?.next().ok_or_else( || anyhow! ("no needed pair"))?;
            assert_eq!(parsed_data.as_str(), input);
        }
        Ok(())
    }
    ...
    fn test_rfact_on_correct() -> anyhow::Result<()> {
        ...
        for input in correct_inputs {
            let parsed_data = RegexGrammar::parse(Rule::rfact, input)?.next().ok_or_else( || anyhow! ("no needed pair"))?;
            assert_eq!(parsed_data.as_str(), input);
        }
        Ok(())    
    }
    ...
    fn test_rterm_on_correct() -> anyhow::Result<()> {
        let correct_inputs = ["ab","b+a", "bab*", "a*b?", "(a|t)", "(ab)b", "(a*)?", "(n+|b)+", "a(b+)"];
        for input in correct_inputs {
            let parsed_data = RegexGrammar::parse(Rule::rterm, input)?.next().ok_or_else( || anyhow! ("no needed pair"))?;
            assert_eq!(parsed_data.as_str(), input);
        }
        Ok(())  
    }
    ...
    fn test_rterm_on_incorrect() -> anyhow::Result<()> {
        let incorrect_inputs = ["|a","*b", "?.", "+,", "(т", ")а", "(н+(z)", "(a|())", "(ab+"];
        for input in incorrect_inputs {
    ...
    fn test_rexpr_on_correct() -> anyhow::Result<()> {
        ...
        for input in correct_inputs {
            let parsed_data = RegexGrammar::parse(Rule::rexpr, input)?.next().ok_or_else( || anyhow! ("no needed pair"))?;
            assert_eq!(parsed_data.as_str(), input);
        }
        Ok(())  
    }
    ...
    fn test_reg_on_correct() -> anyhow::Result<()> {
        let correct_inputs = ["(a|b)*c","(x|y)(1|2)", "x'*", "(ab|c)*", "(a?)a", "(ab)?d+"];
        for input in correct_inputs {
            let parsed_data = RegexGrammar::parse(Rule::reg, input)?.next().ok_or_else( || anyhow! ("no needed pair"))?;
            assert_eq!(parsed_data.as_str(), input);
        }
        Ok(())  
    }
    ...
    fn test_reg_on_incorrect() -> anyhow::Result<()> {
        let incorrect_inputs = ["", "(", ")", "*", "+", "?", "|", "a||b", "a|", "a**", "a++", "a??", "(a|b|c", "|a|b", "a|b+(", "a?*", "ab(+)"];
        for input in incorrect_inputs {
